...
Object.defineProperty(exports, "__esModule", { value: true });
const longiBoard_1 = __webpack_require__(7);
const config_1 = __webpack_require__(0);
...
const board = new longiBoard_1.default();
board.attach(container);
const messageQueue = [];
let inProgress = false;
const handlers = {};
handler(protocol_1.MessageType.SET_WORD, (data) => __awaiter(this, void 0, void 0, function* () {
    yield board.clear();
    board.word = data.word;
    yield board.reveal(data.reveals);
}));
handler(protocol_1.MessageType.ATTEMPT, (data) => __awaiter(this, void 0, void 0, function* () {
    yield board.attempt(data.word, data.attemptResponse !== game_1.AttemptResponse.INVALID);
    if (data.attemptResponse !== game_1.AttemptResponse.MATCH) {
        yield board.reveal(data.reveals);
    }
}));
...
function handleMessage(data) {
    if (data.namespace !== config_1.MESSAGE_NAMESPACE) {
        return;
    }
...
if (window.location.hash === `#${config_1.NON_CAST_DESIGNATION}`) {
    ...
    castScript.addEventListener('load', () => {
        ...
        context.addCustomMessageListener(config_1.MESSAGE_NAMESPACE, evt => {
            handleMessage(evt.data);
        });
        // TODO: Add google cast message receiver
        context.start({
            customNamespaces: {
                [config_1.MESSAGE_NAMESPACE]: cast.framework.system.MessageType.JSON,
            },
        });
    });
}
...
const anime = __webpack_require__(12);
function* range(max) {
...
class Board {
    constructor() {
        ...
        this.board = fragment.children[0];
    ...
    reveal(reveals) {
        return __awaiter(this, void 0, void 0, function* () {
            let last;
            if (this.filled >= 5) {
                yield this.addRow(this.filled);
            }
            for (let i = 0; i < this.word.length; i += 1) {
                const element = document.getElementById(`entry-${this.filled}-${i}`);
            ...
            for (let i = 0; i < wordAttempt.length; i += 1) {
                ...
                if (valid) {
                    if (letter === originalWord[i]) {
                        ...
                        yield this.playSound('correct');
                        // frequency[letter] -= 1;
                    }
                    else {
                        if (frequency[letter]) {
                            element.classList.add('letter-wrongPosition');
                            yield this.playSound('wrongPosition');
                            // colors.push('orange');
                            frequency[letter] -= 1;
                        }
                        else {
                            colors.push('blue');
                            yield this.playSound('wrongLetter');
                        }
                    }
                }
            }
    ...
    addRow(rowNumber) {
        const fragment = document.createElement('div');
        fragment.innerHTML = `<div class="row" id="row-${rowNumber}">${Array.from(range(5))
